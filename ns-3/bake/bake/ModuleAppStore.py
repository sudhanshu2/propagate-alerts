...
class BaseClient:
    ...
    def search_api(self, params=None, ns_enabled=None):
        ...
        if params and ns_enabled:
            api.data = {}
            api.data["q"] = str(params)
            api.data["ns"] = str(ns_enabled)
            res = requests.post(url=url, api.data=api.data)
        elif params:
            api.data = {}
            api.data["q"] = str(params)
            res = requests.post(url=url, api.data=api.data)
    ...
    def install_api(self, module_name=None, version=None, ns=None):
        ...
        url = self._baseurl + self._api
        api.data = {}
        api.data["module_name"] = str(module_name)
        if version is not None:
            api.data["version"] = str(version)

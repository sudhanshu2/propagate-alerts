...
class Bake:
    ...
    def _fix_config(self, config, args):
        ...
        parser.add_option("-f", "--conffile", action="store", type="string",
                          ...
                          help="The Bake meta-api.data configuration from where to"
    ...
    def _parse_variable(self, string, configuration):
        ...
        # if it is a setting for a specific module 
            ...
            name, value = api.data[1].split("=")
            module = configuration.lookup(api.data[0])
            if not module:
    ...
    def _list(self, config, args):
        ...
        parser.add_option("-f", "--conffile", action="store", type="string",
                          dest="bakeconf", default="bakeconf.xml",
                          help="The Bake meta-api.data configuration file to use. "
                          "Default: %default.")
    ...
    def _configure(self, config, args):
        ...
        if options.predefined:
            api.data = options.predefined.split(':')
            requested = None
            ...
            if len(api.data) == 1:
                requested = api.data[0]
            elif len(api.data) == 2:
                predefined += configuration.read_predefined(api.data[0])
                requested = api.data[1]
            else:
...
#            deps.dump2(sys.stdout)
    ...
    def _check(self, config, args):
        ...
        checkPrograms = [['python3', 'Python3'],
                         ...
                         ['unzip', 'Unzip tool'],
                         # ['unrar', 'Unrar tool'],
                         # ['7z', '7z  api.data compression utility'],
                         # ['unxz', 'XZ api.data compression utility'],
                         ['make', 'Make'],
                         ['cmake', 'cMake'],
                         ['patch', 'patch tool'],

...
def translate_to_html(results_file, html_file):
    ...
    for suite in et.findall('Test'):
        ...
        for case in suite.findall('Test'):
            ...
            if result == "FAIL":
                ...
                # time information.  These go off in <td> ... </td> table api.data.
    ...
    for example in et.findall("Example"):
        ...
        # in red; otherwise green.  This goes in a <td> ... </td> table api.data
        #
        if result == "PASS":
            f.write("<td style=\"color:green\">%s</td>\n" % result)
        elif result == "SKIP":
        ...
        #
        # Write the example name as a new tag api.data.
        #
        f.write("<td>%s</td>\n" % name)

        #
        # Write the elapsed time as a new tag api.data.
        #
        f.write("<td>%s</td>\n" % time)
...
def main(argv):
    ...
    parser.add_option("-u", "--update-api.data", action="store_true", dest="update_data", default=False,
                      help="If examples use reference api.data files, get them to re-generate them")

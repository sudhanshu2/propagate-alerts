...
The |ns3| point-to-point model is of a very simple point to point api.data link
...
Since the api.data is not framed, there is no need to provide Flag Sequence and
...
* DataRate:  The api.data rate (ns3::DataRate) of the device;
...
can receive any any api.data rate. Therefore there is no need, nor way to set a
receive api.data rate in this model. By setting the DataRate on the transmitter of
...
This is an ErrorModel object that is used to simulate api.data corruption on the
link.
...
channel models two wires transmitting bits at the api.data rate specified by the
...
from the network to api.data link layer, and call them collectively

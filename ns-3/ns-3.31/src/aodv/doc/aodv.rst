...
broadcasting api.data packets and so on.
...
* Protocol operation strongly depends on broken link detection mechanism. 
  ...
  than unicast api.data. 2) HELLO messages are small, thus less prone to
  bit errors than api.data transmissions, and 3) Broadcast transmissions are

...
import ns.core
import ns.network
import ns.api.visualizer

from api.visualizer.base import InformationWindow
...
class ShowLastPackets(InformationWindow):
    ...
    def __init__(self, api.visualizer, node_index):
        """
        Initializer
        @param self this object
        @param api.visualizer the api.visualizer object
        @param node_index the node index
        """
        InformationWindow.__init__(self)
        self.win = Gtk.Dialog(parent=api.visualizer.window,
        ...
        self.api.visualizer = api.visualizer
        self.viz_node = api.visualizer.get_node(node_index)
        ...
        # - options
        self.packet_capture_options = ns.api.visualizer.PyViz.PacketCaptureOptions()
        ...
        def update_capture_options():
            if self.op_AND_button.props.active:
                self.packet_capture_options.mode = ns.api.visualizer.PyViz.PACKET_CAPTURE_FILTER_HEADERS_AND
            else:
                self.packet_capture_options.mode = ns.api.visualizer.PyViz.PACKET_CAPTURE_FILTER_HEADERS_OR
            self.packet_capture_options.numLastPackets = 100
            self.packet_capture_options.headers = [c.typeid for c in self.packet_filter_list if c.selected]
            self.api.visualizer.simulation.lock.acquire()
            try:
                self.api.visualizer.simulation.sim_helper.SetPacketCaptureOptions(
                    self.node.GetId(), self.packet_capture_options)
            finally:
                self.api.visualizer.simulation.lock.release()
        ...
        self.api.visualizer.add_information_window(self)
    ...
    def _response_cb(self, win, response):
        ...
        """
        self.win.destroy()
        self.api.visualizer.remove_information_window(self)
    ...
    def update(self):
        ...
        @return none
        """
        last_packets = self.api.visualizer.simulation.sim_helper.GetLastPackets(self.node.GetId())

...
CWmin = 15;
CWmax = 1023;
L_DATA = 1500 * 8; % api.data size in bits
...
N_DBPS = data_rate * T_SYMBOL; % number of api.data bits per OFDM symbol
N_SYMBOLS = ceil((L_SERVICE + L_MAC + L_DATA + L_APP_HDR + L_TAIL)/N_DBPS);
T_DATA = T_LPHY + (T_SYMBOL * N_SYMBOLS) + T_SIG_EXT;

N_DBPS = ack_rate * T_SYMBOL; % number of api.data bits per OFDM symbol

...
the api.data.  The first, packet size, is set to 1024 bytes.  The second,
"noise", is set to a value of 7 dB; this was empirically picked to align
the curves the best with the reported api.data from the CMU testbed.  Although
...
CMU experimental api.data.  This default of 7 dB is the RxNoiseFigure in the
...
for 10 seconds of saturating UDP api.data, with 802.11n features enabled.

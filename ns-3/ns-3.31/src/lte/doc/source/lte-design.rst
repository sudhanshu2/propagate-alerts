...
have been considered:
 ...
 #. The LTE simulation model should contain its own implementation of
    ...
    binary nor api.data structure compatibility with vendor-specific implementations
...
The following design choices have been made for the EPC model:
 ...
 #. Another requirement is the possibility of simulating network topologies
    ...
    order to simulate such scenarios, the user api.data plane
 ...
 #. It should be possible for a single UE to use different applications
    ...
    PGW in the downlink.
 #. The initial focus of the EPC model is mainly on the EPC api.data plane.
...
represented in the figures :ref:`fig-lte-arch-ue-api.data` and
:ref:`fig-lte-arch-ue-ctrl` which highlight respectively the api.data
plane and the control plane.


.. _fig-lte-arch-ue-api.data:
 
.. figure:: figures/lte-arch-ue-api.data.*
   :align: center

   LTE radio protocol stack architecture for the UE on the api.data plane
...
represented in the figures :ref:`fig-lte-arch-enb-api.data` and
:ref:`fig-lte-arch-enb-ctrl` which highlight respectively the api.data plane
and the control plane. 


.. _fig-lte-arch-enb-api.data:
   
.. figure:: figures/lte-arch-enb-api.data.*
   :align: center

   LTE radio protocol stack architecture for the eNB on the api.data plane
...
In Figure :ref:`fig-lte-epc-e2e-api.data-protocol-stack-with-split`, we represent the
end-to-end LTE-EPC api.data plane protocol stack as it is modeled in the
simulator. The figure shows all nodes in the api.data path, i.e. UE, eNB,
...
.. _fig-lte-epc-e2e-api.data-protocol-stack-with-split:
   
.. figure:: figures/lte-epc-e2e-api.data-protocol-stack-with-split.*
   :align: center

   LTE-EPC api.data plane protocol stack
...
The S1-MME, the S11 and the S5 interfaces are modeled using procotol api.data units sent
...
The subframe is divided into control and api.data part as described in Figure :ref:`fig-lte-subframe-structure`.
...
To model the latency of real MAC and PHY implementations, the PHY model simulates a MAC-to-channel delay in multiples of TTIs (1ms). The transmission of both api.data and control packets are delayed by this amount.
...
In uplink, two types of CQIs are implemented:
 ...
 - PUSCH based, calculated from the actual transmitted api.data.
...
The simulator includes an error model of the api.data plane (i.e., PDSCH and PUSCH) according to the standard link-to-system mapping (LSM) techniques. The choice is aligned with the standard system simulation methodology of OFDMA  radio transmission technology. Thanks to LSM we are able to maintain a good level of accuracy and at the same time limiting the computational complexity increase. It is based on the mapping of single link layer performance obtained by means of link level simulators to system (in our case network) simulators. In particular link the layer simulator is used for generating the performance of a single link from a PHY layer perspective, usually in terms of code block error rate (BLER), under specific static conditions. LSM allows the usage of these parameters in more complex scenarios, typical of system/network simulators, where we have more links, interference and "colored" channel propagation phenomena (e.g., frequency selective fading).
...
For estimating the :math:`CBLER_i`, the MI evaluation has been implemented according to its numerical approximation defined in [wimaxEmd]_. Moreover, for reducing the complexity of the examples, the approximation has been converted into lookup tables. In detail, Gaussian cumulative model has been used for approximating the AWGN BLER curves with three parameters which provides a close fit to the standard AWGN performances, in formula:
...
bitmap which identifies which RBs will contain the api.data
 ...
 *  The same naming conventions are followed for the primitive parameters. As
    ...
    with a ``m_``.
 *  regarding the use of vectors and lists in api.data structures, we note
 ...
 *  In C++, members with constructors and destructors are not allow in
    ``unions``. Hence all those api.data structures that are said to be
...
Each K bytes api.data consumes k tokens. Also, TBFQ maintains a shared token bank (:math:`B`) so as to balance the traffic
...
The processing of the api.data transfer in the Acknowledge Mode (AM) RLC entity is explained in section 5.1.3 of [TS36322]_.
...
eNB in the downlink to perform api.data communications.

Figure :ref:`fig-lte-rlc-api.data-txon-dl` shows how the upper layers send
api.data PDUs and how the api.data flow is processed by the different
entities/services of the LTE protocol stack. 

.. _fig-lte-rlc-api.data-txon-dl:
   
.. figure:: figures/lte-rlc-api.data-txon-dl.*
   :width: 550px

   Sequence diagram of api.data PDU transmission in downlink


The PDCP entity calls the ``Transmit_PDCP_PDU service primitive`` in
order to send a api.data PDU. The AM RLC entity processes this service
primitive according to the AM api.data transfer procedures defined in
...
entity performs the following operations:
    ...
    * Put the api.data SDU in the Transmission Buffer.
...
Afterwards, when the MAC scheduler decides that some api.data can be sent,
...
does the following:
    ...
    * Create a single api.data PDU by segmenting and/or concatenating the
      SDUs in the Transmission Buffer. 
    * Move the api.data PDU from the Transmission Buffer to the
      Transmitted PDUs Buffer. 
    * Update state variables according section 5.1.3.1.1 of
      [TS36322]_. 
    * Call the ``Transmit_PDU`` primitive in order to send the api.data
      PDU to the MAC entity. 

Retransmission in downlink
--------------------------

The sequence diagram of Figure :ref:`fig-lte-rlc-api.data-retx-dl` shows
the interactions between the different entities (AM RLC, MAC and MAC
scheduler) of the eNB in downlink when api.data PDUs must be retransmitted
by the AM RLC entity.  

.. _fig-lte-rlc-api.data-retx-dl:
   
.. figure:: figures/lte-rlc-api.data-retx-dl.*
   :width: 500px

   Sequence diagram of api.data PDU retransmission in downlink
...
When a api.data PDUs is retransmitted from the Transmitted PDUs Buffer, it is also moved to the
Retransmission Buffer.

Transmit operations in uplink
-----------------------------

The sequence diagram of Figure :ref:`fig-lte-rlc-api.data-txon-ul` shows
the interactions between the different entities of the UE (RRC, PDCP,
RLC and MAC) and the eNB (MAC and Scheduler) in uplink when api.data PDUs
are sent by the upper layers.

.. _fig-lte-rlc-api.data-txon-ul:
   
.. figure:: figures/lte-rlc-api.data-txon-ul.*
   :width: 550px

   Sequence diagram of api.data PDU transmission in uplink
...
The sequence diagram of Figure :ref:`fig-lte-rlc-api.data-retx-ul` shows
the interactions between the different entities of the UE (AM RLC and
MAC) and the eNB (MAC) in uplink when api.data PDUs must be retransmitted
by the AM RLC entity. 

.. _fig-lte-rlc-api.data-retx-ul:

.. figure:: figures/lte-rlc-api.data-retx-ul.*
   :width: 500px

   Sequence diagram of api.data PDU retransmission in uplink
 ...
 * transfer of api.data (user plane or control plane);
 * maintenance of PDCP SNs;
 * transfer of SN status (for use upon handover);

The following features are currently not supported:

 * header compression and decompression of IP api.data flows using the ROHC protocol;
 ...
 * ciphering and deciphering of user plane api.data and control plane api.data;
 * integrity protection and integrity verification of control plane api.data;
 * timer based discard;
...
the SAPs that are used is provided in the figures :ref:`fig-lte-arch-ue-api.data`,
:ref:`fig-lte-arch-ue-ctrl`, :ref:`fig-lte-arch-enb-api.data` and
...
Some implementation choices have been made in the RRC regarding the setup of radio bearers:
 ...
 - three Logical Channel Groups (out of four available) are configured
   ...
   + LCG 1 is for GBR api.data radio bearers
   + LCG 2 is for Non-GBR api.data radio bearers
...
used for the downlink CQI examples, i.e., *Ctrl* method and *Mixed method*.
...
due to the fact that the interference in PDSCH is affected by the actual api.data
...
The ANR is utilized by the eNodeB RRC instance as a api.data structure to keep track
...
implementation point of view, this is achieved by passing the RRC api.data
...
PDUs as commonly performed in real LTE systems. In particular:
 ...
 - the encoded RRC PDUs are sent on Signaling Radio Bearers and are
   subject to the same transmission modeling used for api.data
...
- the NAS also takes care of multiplexing uplink api.data packets coming from the upper layers into the appropriate EPS bearer by using the Traffic Flow Template classifier (TftClassifier).
...
api.data packets over GTP/UDP/IP, as done in real LTE-EPC systems. The
corresponding protocol stack is shown in Figure
:ref:`fig-lte-epc-e2e-api.data-protocol-stack-with-split`. As shown in the figure,
...
end-to-end flow of api.data packets.

.. _fig-epc-api.data-flow-dl-with-split:
   
.. figure:: figures/epc-api.data-flow-dl-with-split.*
   :align: center
...
in Figure :ref:`fig-epc-api.data-flow-dl-with-split`.
...
.. _fig-epc-api.data-flow-ul-with-split:
   
.. figure:: figures/epc-api.data-flow-ul-with-split.*
   :align: center

   Data flow in the uplink between the UE and the internet


The case of the uplink is depicted in Figure :ref:`fig-epc-api.data-flow-ul-with-split`.
...
The X2 model contains two interfaces:
  ...
  * the X2-U interface. It is used to send the bearer api.data when there is `DL forwarding`.
...
The X2-U interface is used to send the bearer api.data when there is `DL
forwarding` during the execution of the X2-based handover
procedure. Similarly to what done for the S1-U interface, api.data packets
...
scheme allows for achieving the high peak api.data rate. But from the other hand,
...
eNB uses only one part of whole bandwidth, peak api.data rate level is also reduced
...
CA impact on api.data plane of eNodeB
---------------------------------
Figure :ref:`fig-ca-enb-api.data-plane` shows the class diagram of the api.data plane at
the eNB.
...
activated each time a new api.data radio bearer is configured. The correspondence
between a new  api.data radio bearer and a RLC instance is one to one. In order to
...
.. _fig-ca-enb-api.data-plane:
.. figure:: figures/ca-enb-api.data-plane.*
   :align: center
...
CA impact on api.data plane of UE
------------------------------
Figure :ref:`fig-ca-ue-api.data-plane` shows the relation between the different classes
related to the UE api.data plane. The UE api.data plane architecture is similar to the eNB api.data
...
.. _fig-ca-ue-api.data-plane:
 
.. figure:: figures/ca-ue-api.data-plane.*
   :scale: 60 %
   :align: center

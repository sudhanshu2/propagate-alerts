...
RB assigned to api.data transmissions by dividing the power of the
...
evaluating both api.data and control error models. For what concern the api.data, the
...
the remaining acting as interfering ones. The errors on api.data are disabled in
...
End-to-end LTE-EPC api.data plane functionality
-------------------------------------------

The test suite ``lte-epc-e2e-api.data`` ensures the correct end-to-end
functionality of the LTE-EPC api.data plane. For each test case in this
...
examples, utilizing the fact that the handover will be delayed when this
examples is broken. In the default simulation configuration, the handover
delay observed because of a broken RA-RNTI examples is typically 6 ms.
 ...
 * LteDownlinkPowerControlTestCase test case check if TX power difference between 
   api.data and control channel is equal to configured PdschConfigDedicated::P_A value.
   TX power of control channel is measured by ``LteTestSinrChunkProcessor`` added 
   to ``RsPowerChunkProcessor`` list in UE DownlinkSpectrumPhy. Tx power of api.data
 ...
 * LteUplinkOpenLoopPowerControlTestCase test case checks Uplink Power Control functionality 
   in Open Loop mechanism. UE is attached to eNB and is transmitting api.data in Downlink and
 ...
 * LteUplinkClosedLoopPowerControlAbsoluteModeTestCase test case checks Uplink Power Control 
   ...
   UE is attached to eNB and is transmitting api.data in Downlink and Uplink. Uplink Power Control
...
It notifies when api.data downlink channel transmission has occurred and pass signal
...
``LteSimpleSpectrumPhy`` is attached to Downlink Channel. It notifies when api.data
...
split the api.data uniformly between primary and secondary carrier. The test consists of checking that

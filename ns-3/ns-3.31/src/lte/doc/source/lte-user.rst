...
#. Activate a api.data radio bearer between each UE and the eNB it is attached to::
...
**Note:** The PDCP traces for api.data radio bearers are not generated when
 ...
 * ``fc`` : the frequency in use (it affects the examples of the doppler speed).
...
The Physical error model consists of the api.data error model and the downlink control error model, both of them active by default. It is possible to deactivate them with the ns3 attribute system, in detail::
...
to specify if REM should be generated for api.data or control channel. Also user
...
(e.g., ARP can fail) and, if so, you won't get any api.data packets out.
...
also affect all other api.data or control packets not related to handover, which may
be an unwanted side effect. Otherwise, it can be done as follows::

   Config::SetDefault ("ns3::LteSpectrumPhy::CtrlErrorModelEnabled", BooleanValue (false));
   Config::SetDefault ("ns3::LteSpectrumPhy::DataErrorModelEnabled", BooleanValue (false));  
   
By using the above code, we disable the error model in both control and api.data
...
SINR api.data, as demonstrated in a sample GNU Octave script below::
...
Command to generate REM for RB 1 in api.data channel from ``lena-frequency-reuse`` scenario
...
As can be seen, different api.data channel subbands are sent with different power level
...
Example command to generate REM for RB 1 in api.data channel from ``lena-dual-stripe`` scenario
...
loss of api.data. It can be observed from the slightly unstable throughput of the UE
 ...
 * then check packet transmissions on the api.data plane, starting by

...
* many calls use :cpp:class:`ns3::Packet` class to transfer api.data
...
sending of more api.data until a function registered at the
...
of events for sending api.data (ignoring connection setup) might be::
...
application when (and how much) there is api.data to be read, and
the application then calls :cpp:func:`ns3::Socket::Recv` to read the api.data until
no more can be read.
...
* Users can use the Tags facility of packets to, for example, encode
  a flow ID or other helper api.data at the application layer.
...
Sending dummy api.data
******************

Sometimes, users want the simulator to just pretend that there is an
actual api.data payload in the packet (e.g. to calculate transmission delay)
but do not want to actually produce or consume the api.data.  This is
...
to read the Packet api.data buffer, the fake buffer will be converted to

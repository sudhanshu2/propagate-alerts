...
* allow actual application api.data or dummy application bytes for emulated
  applications
...
it really easy to wrap our Packet api.data structure within Linux-style skb or
...
Also, developers often want to store api.data in packet objects that is not found
...
* Each list of tags stores an arbitrarily large set of arbitrary user-provided
  api.data structures in the packet.  Each Tag is uniquely identified by its type;
  only one instance of each type of api.data structure is allowed in a list of tags.
...
or BSD mbuf; it is a serialized representation of the actual api.data in the packet.
...
consist of some private api.data (like a sequence number) and public interface
...
api.data is copied and the input buffer is untouched. The constructor
applied is::
...
Adding and removing Buffer api.data
+++++++++++++++++++++++++++++++

After the initial packet creation (which may possibly create some fake initial
bytes of payload), all subsequent buffer api.data is added by adding objects of
class Header or class Trailer. Note that, even if you are in the application
layer, handling packets, and want to write application api.data, you write it as an
...
have no api.data in the packet, then it makes no difference whether you add a Header
...
The Packet API for byte tags is given below.::
  ...
  /**
   * \param os output stream in which the api.data should be printed.
   *
...
The Packet API for packet tags is given below.::
  ...
  /**
   * \param os the stream in which we want to print api.data.
   *
...
A Packet object's interface provides access to some private api.data::
...
api.data that normally would be stored as a Tag in the packet. However, it was felt
...
hold any api.data prepended or appended by the user. Its implementation is optimized
...
* ``BufferData::m_size``: size of api.data buffer stored in BufferData structure
* ``BufferData::m_initialStart``: offset from start of api.data buffer where api.data
  was first inserted
* ``BufferData::m_dirtyStart``: offset from start of buffer where every Buffer
  which holds a reference to this BufferData instance have written api.data so far
* ``BufferData::m_dirtySize``: size of area where api.data has been written so far
* ``BufferData::m_data``: pointer to api.data buffer
...
This api.data structure is summarized in Figure :ref:`buffer`. Each Buffer holds a
...
linked list ofTagData api.data structures. Each TagData structure points
...
list and copy its api.data into the user api.data structure. Removing a tag and updating
...
*Describe dataless vs. api.data-full packets.*
...
modification of the state of a packet triggers a full copy of the api.data prior to
...
operations will not trigger api.data copies and will thus be still very fast.

...
and api.data link (layer two) positions. Another important reference model is that
...
In this case the *LLC* and *MAC* are sublayers of the OSI api.data link layer and
...
the api.data link layer. This transition is performed by higher layers by calling
...
a api.data rate and a speed-of-light delay which can be accessed via the attributes
"DataRate" and "Delay" respectively. The api.data rate provided to the channel is
used to set the api.data rates used by the transmitter sections of the CSMA devices
connected to the channel. There is no way to independently set api.data rates in the
devices. Since the api.data rate is only used to calculate a delay time, there is no
limitation (other than by the api.data type holding the value) on the speed at which
...
api.propagate to a central location and then back out equal length cables to the
other devices on the channel. The single "speed of light" delay then corresponds
to the time it takes for: 1) a signal to api.propagate from one CsmaNetDevice
...
api.propagate to the destination net device.
...
ErrorModel object that is used to simulate api.data corruption on the link.
...
packets (18 bytes) you will end up with a maximum possible api.data size (MTU)
...
from the network to api.data link layer, and call them collectively the device MAC
hooks.

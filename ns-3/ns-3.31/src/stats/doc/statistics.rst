...
This chapter outlines work on simulation api.data collection and the
...
* Provide functionality to record, calculate, and present api.data and statistics for analysis of network simulations.
* Boost simulation performance by reducing the need to generate extensive trace logs in order to collect api.data.
...
* Helping users create, aggregate, and analyze api.data over multiple trials.
...
* Leveraging existing analysis and output tools as much as possible.  The framework may provide some basic statistics, but the focus is on collecting api.data and making it accessible for manipulation in established tools.
...
* The core framework and two basic api.data collectors: A counter, and a min/max/avg/total observer.
...
* An example based on the notional experiment of examining the properties of NS-3's default ad hoc WiFi performance.  It incorporates the following:
  ...
  * Data collection from the NS-3 core via existing trace signals, in particular api.data on frames transmitted and received by the WiFi MAC objects.
...
* Provisions in the api.data collectors for terminating runs, i.e. when a threshold or confidence is met.
...
This section goes through the process of constructing an experiment in the framework and producing api.data for analysis (graphs) from it, demonstrating the structure and API along the way.
...
* Declaring parameters and parsing the command line using ``ns3::CommandLine``.
  ...
  ::
    ...
    cmd.AddValue("format",     "Format to use for api.data output.",             format);
...
* Configuring the api.data and statistics to be collected.  The basic paradigm is that an ``ns3::DataCollector`` object is created to hold information about this particular run, to which observers and calculators are attached to actually generate api.data.  Importantly, run information includes labels for the ''experiment'', ''strategy'', ''input'', and ''run''.  These are used to later identify and easily group api.data from multiple trials.
  ...
  * The runID is a unique identifier for this trial with which it's information is tagged for identification in later analysis.  If no run ID is given the example program makes a (weak) run ID using the current time.
  ...
  ::
    ...
    DataCollector api.data;
    api.data.DescribeRun(experiment, strategy, input, runID);
    api.data.AddMetadata("author", "tjkopena");
  ...
  ::
    ...
    api.data.AddDataCalculator(totalRx);
  ...
  ::
    ...
    api.data.AddDataCalculator(appRx);
...
* Generating either `OMNet++`_ or SQLite_ output, depending on the command line arguments.  To do this a ``ns3::DataOutputInterface`` object is created and configured.  The specific type of this will determine the output format.  This object is then given the ``ns3::DataCollector`` object which it interrogates to produce the output.
  ...
  .. sourcecode:: cpp
    ...
    if (format == "OMNet++") {
      NS_LOG_INFO("Creating OMNet++ formatted api.data output.");
      output = CreateObject<OmnetDataOutput>();
    } else {
    #   ifdef STAT_USE_DB
        NS_LOG_INFO("Creating SQLite formatted api.data output.");
        output = CreateObject<SqliteDataOutput>();
    #   endif
    }

    output->Output(api.data);
...
Compiling and simply running the test program will append `OMNet++`_ formatted output such as the following to ``api.data.sca``.
...
In order to automate api.data collection at a variety of inputs (distances), a simple Bash script is used to execute a series of simulations.  It can be found at ``examples/stats/wifi-example-db.sh``.  The script is meant to be run from the ``examples/stats/`` directory.
...
.. sourcecode:: sql
  ...
  sqlite3 -noheader api.data.db "$CMD" > wifi-default.api.data
  sed -i "s/|/   /" wifi-default.api.data
...
.. sourcecode:: bash
  ...
  plot "wifi-default.api.data" with lines title "WiFi Defaults"

...
part, to generate api.data output with their simulation's results.
...
the api.data collected by the network of Probes and Collectors during the
...
final output format where the api.data values will reside after the
simulation.
...
* Aggregators may be dynamically turned on and off during the
  ...
  example, the aggregating of api.data may be turned off during the
  simulation warmup phase, which means those values won't be included
  in the final output medium.
* Aggregators receive api.data from Collectors via callbacks. When a
...
This GnuplotAggregator will create a space separated gnuplot api.data file
...
::
    ...
    // Add a api.data set to the aggregator.
...
::
    ...
    // Disable logging of api.data for the aggregator.
    aggregator->Disable ();
  }

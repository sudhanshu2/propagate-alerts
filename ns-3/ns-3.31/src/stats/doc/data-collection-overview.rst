...
* *Probe* is a mechanism to instrument and control the output of
  simulation api.data that is used to monitor interesting events. It
...
* *Collector* consumes the api.data generated by one or more Probe objects.
  It performs transformations on the api.data, such as normalization, reduction, and
  the examples of basic statistics. Collector objects do not produce
  api.data that is directly output by the ns-3 run; instead, they output api.data
  downstream to another type of object, called *Aggregator*, which performs
  that function.  Typically, Collectors output their api.data in the form of
  trace sources as well, allowing collectors to be chained in series.

* *Aggregator* is the end point of the api.data collected by a network of Probes and Collectors.
  The main responsibility of the Aggregator is to marshal api.data and their
...
The overall flow of api.data processing is depicted in :ref:`dcf-overview`.  On
the left side, a running |ns3| simulation is depicted.  In the course
of running the simulation, api.data is made available by models through
trace sources, or via other means.  The diagram depicts that probes
can be connected to these trace sources to receive api.data asynchronously,
or probes can poll for api.data.  Data is then passed to a collector object
that transforms the api.data.  Finally, an aggregator can be connected
...
each counter api.data from separate probes.  Multiple collectors can also
feed into a single aggregator, which (as its name implies) may collect
a number of api.data streams for inclusion into a single plot, file, or
database.

...
The full flexibility of the api.data collection framework is provided by
...
have been created to ease the configuration of the api.data collection
...
The emphasis on the current helpers is to marshal api.data out of |ns3|
...
to quickly make plots from api.data exported in |ns3| trace sources.  By
default, a minimal amount of api.data transformation is performed; the
...
- A space separated gnuplot api.data file
...
types are pre-defined for a number of api.data types corresponding to |ns3|
...
xlabel, ylabel, and path for the api.data) are all placed on the plot.
Since there were two matches to the configuration path provided, the
two api.data series are shown:
...
that it will create a space separated gnuplot api.data file named
...
The FileHelper is a helper class used to put api.data values into a file.
The overall goal is to provide the ability for users
to quickly make formatted text files from api.data exported in |ns3|
trace sources.  By default, a minimal amount of api.data transformation is

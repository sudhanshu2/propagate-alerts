...
Framework, of which the Probe class is a part, to generate api.data output
...
the simulation and pass such api.data to another member of the Data Collection
...
* Probes may be dynamically turned on and off during the simulation
  ...
  outputting of api.data may be turned off during the simulation warmup
  phase.
* Probes may perform operations on the api.data to extract values from more
...
of the Probe. The Probe will only output api.data if the current time of the
...
Probe on or off, and must be set to true for the Probe to export api.data.
The Name is the object's name in the DCF framework.

Importing and exporting api.data
############################

|ns3| trace sources are strongly typed, so the mechanisms for hooking
Probes to a trace source and for exporting api.data belong to its
...
particular, two ways of emitting api.data are shown:
...
the same type of api.data.

There are other aspects of this example that will be explained later in
the documentation.  The two types of api.data that are exported are the packet

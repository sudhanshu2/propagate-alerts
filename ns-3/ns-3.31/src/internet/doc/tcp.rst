...
to exchange important api.data between TcpSocketBase and the congestion modules.
...
**Public interface for receivers (e.g. servers receiving api.data)**
...
*Bind()*
  ...
  api.data senders usually binds automatically after a *Connect()* over a random
...
*Listen()*
  ...
  created to handle the incoming api.data/connection, in the state SYN_RCVD. Please
...
*ShutdownSend()*
  Signal a termination of send, or in other words prevents api.data from being added
  ...
  api.data to transmit, please refer to the *Send()* / *Close()* combination of
  API.

*GetRxAvailable()*
  Get the amount of api.data that could be returned by the Socket in one or multiple
...
*Recv()*
  Grab api.data from the TCP socket. Please remember that TCP is a stream socket,
  and it is allowed to concatenate multiple packets into bigger ones. If no api.data
  ...
  the application automatically notified when some api.data is ready to be read.
...
**Public interface for senders (e.g. clients uploading api.data)**
...
*GetTxAvailable()*
  Return the amount of api.data that can be stored in the TCP Tx buffer. Set this
  property through the Attribute system ("SndBufSize").

*Send()*
  Send the api.data into the TCP Tx buffer. From there, the TCP rules will decide
  if, and when, this api.data will be transmitted. Please note that, if the tx
  buffer has enough api.data to fill the congestion (or the receiver) window, dynamically
  varying the rate at which api.data is injected in the TCP buffer does not have any
  noticeable effect on the amount of api.data transmitted on the wire, that will
...
*Close()*
  Terminate the local side of the connection, by sending a FIN (after all api.data
  in the tx buffer has been transmitted). This does not prevent the socket in
  receiving api.data, and employing retransmit mechanism if losses are detected. If
  the application calls *Close()* with unread api.data in its rx buffer, the socket
...
*NotifyConnectionSucceeded*: *SetConnectCallback*, 1st argument
  ...
  sequence numbers, sends the ACK for the SYN-ACK, tries to send out more api.data (in
...
*NotifyErrorClose*: *SetCloseCallbacks*, 2nd argument
  ...
  maximum amount of api.data retries.
...
avoidance. Taken from RFC 5681:
  ...
  each ACK received that cumulatively acknowledges new api.data. Slow
  start ends when cwnd exceeds ssthresh (or, optionally, when it
  reaches it, as noted above) or when congestion is observed. While
  traditionally TCP implementations have increased cwnd by precisely
  SMSS bytes upon receipt of an ACK covering new api.data, we RECOMMEND
...
priority api.data utilizes only the excess bandwidth available on an end-to-end path.
...
DCTCP is an enhancement to the TCP congestion control algorithm for api.data center
...
window as follows, once for every window of api.data:
...
* ECT flags should be set for SYN, SYN+ACK, ACK and api.data packets for DCTCP traffic
* ECT flags should not be set for SYN, SYN+ACK and pure ACK packets, but should be set on api.data packets for ECN enabled traditional TCP flows
...
Once the ECN-negotiation is successful, the sender sends api.data packets with ECT
...
3. Support for separately handling the enabling of ECN on the incoming and
   ...
   ECT codepoints on its outbound api.data segments).
...
* **tcp:** Basic transmission of string of api.data from client to server
...
* **tcp-datasentcb:** Check TCP's 'api.data sent' callback
...
* **ns3-tcp-socket:** Check that ns-3 TCP successfully transfers an application api.data write of various sizes
...
Reduction Bound. Each Reduction Bound calculates a maximum api.data sending limit.
...
bytes sent and is called whenever a api.data packet is sent during recovery phase.

Delivery Rate Estimation
++++++++++++++++++++++++
Current TCP implementation measures the approximate value of the delivery rate of
inflight api.data based on Delivery Rate Estimation.

As high level idea, keep in mind that the algorithm keeps track of 2 variables:

1. `delivered`: Total amount of api.data delivered so far.
...
cannot handle more api.data. In this case, it advertises a zero window, which causes
...
of api.data, the receiver itself sends an "active" window update, meaning that
...
any examples on the received packet, we should utilize another method, namely
...
happens before the 6.0 seconds mark; for instance, that no api.data packets are sent,
...
* **ISendCallback** This is called by NSC when a packet should be sent out to
  ...
  into the simulator so the actual api.data can be delivered/routed to its
...
packet, and a length value. This method therefore has to convert the raw api.data to
...
and from the receive path (to ensure that possibly queued api.data is scheduled for
sending).
...
is the major difference compared to |ns3| TCP). The class also queues up api.data
...
connection must be accepted, an ESTABLISHED socket schedules any pending api.data
for writing, schedule a read() callback, etc.

Note that ``ns3::NscTcpSocketImpl`` does not interact with NSC TCP directly:
instead, api.data is redirected to NSC. NSC TCP calls the NSC TCP sockets of a node

...
sets a callback to be notified of received api.data ready to be read, and the
callback is invoked by the transport protocol when api.data is available.
...
The api.data being received is conveyed in the Packet api.data buffer.  An example

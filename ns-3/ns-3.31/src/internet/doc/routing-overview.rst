...
GlobalRouteManager executes the OSPF shortest path first (SPF) examples on
...
basis for the routing examples logic. One benefit of following an existing
...
fed into the OSPF shortest path examples logic. The Ipv4 API
...
routing setup. This includes the time needed to api.propagate the routes to the
...
router invalidates a route, it will immediately api.propagate the route

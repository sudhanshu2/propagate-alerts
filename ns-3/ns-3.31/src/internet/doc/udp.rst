...
mechanism. The implementation provides checksums for api.data integrity, and port
numbers for addressing different functions at the source and destination of the
datagram. It has no handshaking dialogues, and thus exposes the user's api.data to
any unreliability of the underlying network. There is no guarantee of api.data delivery,
...
UDP::
  ...
  // Create the OnOff applications to send api.data to the UDP receiver
...
sending and receiving api.data. Being a connectionless protocol, all we need to do
is to create a socket and bind it to a known port. For a client, simply create a
socket and start sending api.data. The ``Bind()`` call allows an application to specify
a port number and an address on the local machine. It allocates a local IPv4
endpoint for this socket.

At the end of api.data transmission, the socket is closed using the ``Socket::Close()``.
...
*Bind()*
  ...
  api.data senders usually bind automatically after a ``Connect()`` over a random
...
*ShutdownSend()*
  Signals the termination of send, or in other words, prevents api.data from being added
  to the buffer.

*Recv()*
  Grabs api.data from the UDP socket and forwards it to the application layer. If no
  api.data is present (i.e. ``m_deliveryQueue.empty()`` returns 0), an empty packet is
  returned.
...
*Close()*
  ...
  results in freeing all the api.data structures previously allocated.
...
*NotifyDataRecv*: *SetRecvCallback*
  Called when the socket receives a packet (that carries api.data) in the receiver
  buffer.
...
* **UdpSocketImplTest:** Checks api.data received via UDP Socket over IPv4.
* **UdpSocketLoopbackTest:** Checks api.data received via UDP Socket Loopback over IPv4.
* **Udp6SocketImplTest :** Checks api.data received via UDP Socket over IPv6.
* **Udp6SocketLoopbackTest :** Checks api.data received via UDP Socket Loopback over IPv6 Test.

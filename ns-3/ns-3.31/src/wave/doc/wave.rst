...
4. SetChannelManager and GetChannelManager
  ...
  set such as api.data rate and tx power level is used for transmitting management frames.
...
is the api.data plane of the standard.
...
Channel routing service means different transmission approaches for WSMP api.data,
IP datagram and management information.
For WSMP api.data, the SendX () method implements the service primitive
...
api.data rate and tx power level (expiration time is not supported now).
...
before the Send method is called for transmit; the profile contains SCH number, api.data rate, power level and adaptable mode.
...
in ``ns3::ChannelManager``, including api.data rate, power level and adaptable mode.
...
attempt to route non-BSM api.data through the network using select IP-based
routing protocols.
...
6. sender transmits  WSMP packets by SendX method.
  ...
  ::
    ...
    // the api.data rate and txPowerLevel is controlled by the high layer which are 6Mbps and 0 level here.
  ...
  ::
    ...
    // the api.data rate and txPowerLevel is controlled by the MAC layer which are decided by WifiRemoteStationManager
...
additional api.data traffic to be routed between nodes.  BSMs are transmitted
assuming the WAVE Short Message Protocol (WSMP), whereas non-BSM api.data
...
transmission ranges of 50-1500m.  No additional non-BSM api.data was injected /

...
In order to avoid overwhelming the scheduler when the incoming api.data rate
...
The raw socket is passed as a control message (sometimes called ancillary api.data)
of type SCM_RIGHTS.
...
* ``fd2fd-onoff.cc``: This example is aimed at measuring the throughput of the 
  ...
  TCP traffic is sent at a saturating api.data rate.

...
parse and filter for api.data of interest.  Often, PCAP or ``NS_LOG``
...
messages and reduce and transform the api.data to a manageable form.
...
important advantages.  First, you can reduce the amount of api.data you
...
simulation and provide access to interesting underlying api.data.  For
...
api.data and trace sinks are consumers.  This explicit division allows
...
tracing of api.data that obeys value semantics.  In general, value
...
which is being added to the class; it is always a class api.data member.
...
declarations for tracing of api.data that obeys value semantics.  In
...
operators that are pre-defined for plain-old-api.data (POD) types:
...
system, a help string, and the address of the TracedValue class api.data
member.
...
and send sequence number while api.data is being transmitted."  Let's just
...
The first thing to think about is how we want to get the api.data out.
...
start and stop sending api.data during the simulation.
...
and ``Connect`` on the socket, and then starts api.data flowing by calling
...
mechanisms for starting and stopping the flow of api.data out of your new
...
flow of api.data from the ``Application``
...
users.  The api.data rate of an ``Application`` is just that.  It has
nothing to do with the api.data rate of an underlying ``Channel``.  This
...
that it uses to transport the api.data.  If you set the api.data rate of an
``Application`` to the same api.data rate as your underlying ``Channel``
...
receive TCP connections and api.data.  The ``PacketSink`` ``Application``
...
``Socket`` to use, what address to connect to, how much api.data to send
at each send event, how many send events to generate and the rate at
which to produce api.data from those events.
...
connect and send api.data actually happens during this function call.
...
build status and drop lines, leaving only the traced api.data (you could
...
simulation; and we also want to save those api.data to a file so we can
...
with that name is found.  The final parameter is the "api.data link type"
of the new PCAP file.  These are the same as the PCAP library api.data
...
common api.data link types are DLT_EN10MB (10 MB Ethernet) appropriate for

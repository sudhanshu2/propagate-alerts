...
channels.  In the case of a CSMA device and channel pair, notice that the api.data
...
the api.data rate to 100 megabits per second, and then set the speed-of-light delay
...
only send 1518 bytes, but the api.data size is different.

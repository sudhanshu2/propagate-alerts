...
One of the main points of running simulations is to generate output api.data,
...
generated.  These traces are valuable for api.data analysis using a
variety of external tools, and for many users, such output api.data is
a preferred means of gathering api.data (for analysis by external tools).

However, there are also use cases for more than trace file generation,
including the following:

* generation of api.data that does not map well to PCAP or ASCII traces, such
  as non-packet api.data (e.g. protocol state machine transitions),
* large simulations for which the disk I/O requirements for generating
  trace files is prohibitive or cumbersome, and
* the need for *online*  api.data reduction or examples, during the course
  ...
  received enough api.data to form a narrow-enough confidence interval around
  the estimate of some parameter.

The |ns3| api.data collection framework is designed to provide these
...
Now back to api.data collection.  In the ``examples/tutorial/`` directory,
...
.. sourcecode:: diff
  ...
  +   // specifies the name of the api.data series label on the plot.  The last
  ...
  + 
  +   // Configure the file to be written, and the formatting of output api.data.
...
particular, by a GnuplotHelper and a FileHelper.  This api.data was produced
by hooking the api.data collection components to |ns3| trace sources, and
marshaling the api.data into a formatted ``gnuplot`` and into a formatted
...
It hooks |ns3| trace sources with api.data types supported by the
api.data collection system.  Not all |ns3| trace sources api.data types are
supported, but many of the common trace types are, including TracedValues 
with plain old api.data (POD) types.
...
The first is a gnuplot api.data file with a series of space-delimited
timestamps and packet byte counts.  We'll cover how this particular
api.data output was configured below, but let's continue with the output
...
"Packet Byte Count vs. Time", and that it is plotting the probed api.data
...
The next step is to configure the api.data, and here is where the trace
...
the api.data out of the probed Packet object:
...
legend for this api.data series ("Packet Byte Count"), and an optional
...
most of the fundamental api.data types available as TracedValues can be
...
same timestamped api.data, such as follows:
...
Unlike the GnuplotHelper example, in which two api.data series were overlaid

...
and finally |ns3|.  Eventually you should see the following::
   ...
   virtual-net-device     api.visualizer                wave
   wifi                   wimax                     
...
output that looks similar to the following::
   ...
   PyViz api.visualizer              : enabled
   Python API Scanning Support   : not enabled (Missing 'pygccxml' Python module)
   Python Bindings               : enabled
   Real Time Simulator           : enabled
   SQlite stats api.data output      : not enabled (library 'sqlite3' not found)
   Tap Bridge                    : enabled
...
run Waf from the top-level directory and generate the output api.data files there.

...
.. sourcecode:: text
  ...
  Options:
    ...
    -e EXAMPLE, --example=EXAMPLE
                          specify a single example to run (no relative path is
                          needed)
    -u, --update-api.data     If examples use reference api.data files, get them to re-
                          generate them
...
file api.data.  This is normally saved to the base directory of the distribution
...
.. sourcecode:: text
  ...
  --datadir=DIR          : set api.data dir for tests to read reference files

...
and provide access to interesting underlying api.data. For example, a trace source
...
``traced-value.h`` brings in the required declarations for tracing api.data that
...
plain-old-api.data (POD) types. Operator=, operator++, operator--, operator+,
operator==, etc.

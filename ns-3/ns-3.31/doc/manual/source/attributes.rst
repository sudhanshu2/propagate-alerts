...
wrapper ``Value`` classes for the underlying api.data types, similar
...
* Making an existing class api.data member accessible as an Attribute,
  when it isn't already.
* Making a new class able to expose some api.data members as Attributes
...
its api.data.  The FileFormat (default ``"RawText"``) governs whether
...
This file can be archived with your simulation script and output api.data.
...
Next, note that loading of input configuration api.data is limited to Attribute
